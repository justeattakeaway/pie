...
| `personalizedDescription` | —                                                                      | An optional override for the personalised preference description in the "Manage preferences" modal. Supports plain text or HTML `<a>` or `<pie-link>` tags (e.g. `<a href="...">Privacy Policy</a>`). When omitted or empty, the built-in locale string is used. | `""`        |
