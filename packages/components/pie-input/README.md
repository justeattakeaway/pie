...
| `required` | `boolean` | `false` | If true, the input is required to have a value before submitting the form. The form will autofocus a required field if it is missing a value. Will have no effect if the input is placed inside a form. |
