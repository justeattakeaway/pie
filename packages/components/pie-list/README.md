...
`pie-list` itself is a static container with no selection or keyboard behaviour. To build a **selectable** list, do **not** put the controls in `pie-list`. Instead, place `pie-list-item`s inside a [`pie-radio-group`](https://webc.pie.design/?path=/docs/components-radio-group--overview) (single-select) or a [`pie-checkbox-group`](https://webc.pie.design/?path=/docs/components-checkbox-group--overview) (multi-select), set each item's `selection-type`, and slot the control into the item's `leading` (or `trailing`) slot. Switches have no group, so `selection-type="switch"` rows go directly in a `pie-list`.

#### The `selection-type` prop

`selection-type` is what makes a row selectable. Set it to `radio`, `checkbox` (or `switch`) and it drives the item to:
...
Independent toggles (switches). There is no group, so the rows go in a `pie-list` (which needs its own accessible name), and the switch sits in the `trailing` slot:
...
<pie-list aria-label="Notification settings">
  ...
  <pie-list-item selection-type="switch" primaryText="Push notifications">
    <pie-switch slot="trailing"></pie-switch>
  </pie-list-item>
</pie-list>
```
