...
| Reading `boundingBox` immediately after `toBeVisible()` on an animated element | `toBeVisible` returns `true` mid-animation; position is transient | Motion is already frozen by `BasePage.open` (see above), so the element is at its resting position. If the element was rendered after the page loaded, call `basePage.freezeAnimations()` first |
| A fixed `page.waitForTimeout(...)` to wait for a transition or animation | The duration is a guess; too short under CI load, dead time otherwise | Nothing is needed for motion that started before `open` returned. Motion is settled, so the final state is rendered immediately |
| Mutating the DOM after `load()`, then asserting or snapshotting | The page settled during `load`; the re-render your mutation triggers is not waited for, and an assertion that already passed will not re-wait | `await` the element's `updateComplete` after the mutation, and `basePage.freezeAnimations()` if it rendered new components |
