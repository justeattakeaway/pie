...
test.describe('PieCheckboxGroup - Component tests', () => {
    ...
    test.describe('with list items', () => {
        ...
        test('should not toggle a disabled row when it is clicked', async ({ page }) => {
            await expect.poll(() => isRowDisabled(page, selectors.items[3])).toBe(true);
    ...
    test.describe('with list items in a disabled group', () => {
        ...
        test('should mark every row disabled when the group is disabled', async ({ page }) => {
            await expect.poll(() => isRowDisabled(page, selectors.items[1])).toBe(true);
            await expect.poll(() => isRowDisabled(page, selectors.items[4])).toBe(true);
        });
