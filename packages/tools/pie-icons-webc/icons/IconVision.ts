...
const componentSelector = 'icon-vision';

export class IconVision extends LitElement implements IconProps {
    @property({ type: String, reflect: true })
    public size : Size = 'medium';

    @property({ type: String, reflect: true })
    public class : string = 'c-pieIcon c-pieIcon--vision';
    ...
    connectedCallback () : void {
        if (this._svg?.getAttribute('width') === null) {
            const svgSize = getSvgProps('c-pieIcon c-pieIcon--vision', '', null, 'IconVision');
    ...
    updated (changedProperties: DependentMap<IconProps>) : void {
        ...
        if (changedProperties.has('size')) {
            svgSize = getSvgProps('c-pieIcon c-pieIcon--vision', '', this.size, 'IconVision');
...
customElements.define(componentSelector, IconVision);

declare global {
    interface HTMLElementTagNameMap {
        [componentSelector]: IconVision;
    }
}
