...
const componentSelector = 'icon-atom';

export class IconAtom extends LitElement implements IconProps {
    @property({ type: String, reflect: true })
    public size : Size = 'medium';

    @property({ type: String, reflect: true })
    public class : string = 'c-pieIcon c-pieIcon--atom';
    ...
    updated (changedProperties: DependentMap<IconProps>) : void {
        ...
        if (changedProperties.has('size')) {
            svgSize = getSvgProps('c-pieIcon c-pieIcon--atom', '', this.size, 'IconAtom');
...
customElements.define(componentSelector, IconAtom);

declare global {
    interface HTMLElementTagNameMap {
        [componentSelector]: IconAtom;
    }
}
