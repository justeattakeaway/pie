...
const componentSelector = 'icon-pie-large';

export class IconPieLarge extends LitElement implements IconProps {
    @property({ type: String, reflect: true })
    public size : Size = 'medium';

    @property({ type: String, reflect: true })
    public class : string = 'c-pieIcon c-pieIcon--pieLarge';
    ...
    updated (changedProperties: DependentMap<IconProps>) : void {
        ...
        if (changedProperties.has('size')) {
            svgSize = getSvgProps('c-pieIcon c-pieIcon--pieLarge', '', this.size, 'IconPieLarge');
...
customElements.define(componentSelector, IconPieLarge);

declare global {
    interface HTMLElementTagNameMap {
        [componentSelector]: IconPieLarge;
    }
}
