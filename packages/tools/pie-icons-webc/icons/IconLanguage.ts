...
const componentSelector = 'icon-language';

export class IconLanguage extends LitElement implements IconProps {
    @property({ type: String, reflect: true })
    public size : Size = 'medium';

    @property({ type: String, reflect: true })
    public class : string = 'c-pieIcon c-pieIcon--language';
    ...
    updated (changedProperties: DependentMap<IconProps>) : void {
        ...
        if (changedProperties.has('size')) {
            svgSize = getSvgProps('c-pieIcon c-pieIcon--language', '', this.size, 'IconLanguage');
...
customElements.define(componentSelector, IconLanguage);

declare global {
    interface HTMLElementTagNameMap {
        [componentSelector]: IconLanguage;
    }
}
