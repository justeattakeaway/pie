...
const componentSelector = 'icon-salad';

export class IconSalad extends LitElement implements IconProps {
    @property({ type: String, reflect: true })
    public size : Size = 'medium';

    @property({ type: String, reflect: true })
    public class : string = 'c-pieIcon c-pieIcon--salad';
    ...
    updated (changedProperties: DependentMap<IconProps>) : void {
        ...
        if (changedProperties.has('size')) {
            svgSize = getSvgProps('c-pieIcon c-pieIcon--salad', '', this.size, 'IconSalad');
...
customElements.define(componentSelector, IconSalad);

declare global {
    interface HTMLElementTagNameMap {
        [componentSelector]: IconSalad;
    }
}
