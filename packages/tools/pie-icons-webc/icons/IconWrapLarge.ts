...
const componentSelector = 'icon-wrap-large';

export class IconWrapLarge extends LitElement implements IconProps {
    @property({ type: String, reflect: true })
    public size : Size = 'medium';

    @property({ type: String, reflect: true })
    public class : string = 'c-pieIcon c-pieIcon--wrapLarge';
    ...
    updated (changedProperties: DependentMap<IconProps>) : void {
        ...
        if (changedProperties.has('size')) {
            svgSize = getSvgProps('c-pieIcon c-pieIcon--wrapLarge', '', this.size, 'IconWrapLarge');
...
customElements.define(componentSelector, IconWrapLarge);

declare global {
    interface HTMLElementTagNameMap {
        [componentSelector]: IconWrapLarge;
    }
}
