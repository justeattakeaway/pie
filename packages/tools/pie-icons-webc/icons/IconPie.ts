...
const componentSelector = 'icon-pie';

export class IconPie extends LitElement implements IconProps {
    @property({ type: String, reflect: true })
    public size : Size = 'medium';

    @property({ type: String, reflect: true })
    public class : string = 'c-pieIcon c-pieIcon--pie';
    ...
    updated (changedProperties: DependentMap<IconProps>) : void {
        ...
        if (changedProperties.has('size')) {
            svgSize = getSvgProps('c-pieIcon c-pieIcon--pie', '', this.size, 'IconPie');
...
customElements.define(componentSelector, IconPie);

declare global {
    interface HTMLElementTagNameMap {
        [componentSelector]: IconPie;
    }
}
