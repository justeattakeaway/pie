...
const componentSelector = 'icon-sun-large';

export class IconSunLarge extends LitElement implements IconProps {
    @property({ type: String, reflect: true })
    public size : Size = 'medium';

    @property({ type: String, reflect: true })
    public class : string = 'c-pieIcon c-pieIcon--sunLarge';
    ...
    connectedCallback () : void {
        if (this._svg?.getAttribute('width') === null) {
            const svgSize = getSvgProps('c-pieIcon c-pieIcon--sunLarge', '', null, 'IconSunLarge');
    ...
    updated (changedProperties: DependentMap<IconProps>) : void {
        ...
        if (changedProperties.has('size')) {
            svgSize = getSvgProps('c-pieIcon c-pieIcon--sunLarge', '', this.size, 'IconSunLarge');
...
customElements.define(componentSelector, IconSunLarge);

declare global {
    interface HTMLElementTagNameMap {
        [componentSelector]: IconSunLarge;
    }
}
