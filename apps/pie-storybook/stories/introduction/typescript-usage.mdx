...
Our published components contain `index.d.ts` and `react.d.ts` files that contain the types for the component and its props. These files are automatically generated during the build process and are used by the TypeScript compiler to provide type checking and autocompletion in your IDE. You should not need to import or interact with these. They can be viewed in the `dist` directory of the component packages.
