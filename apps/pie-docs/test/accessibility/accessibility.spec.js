...
expectedRoutesJson.forEach((route) => {
    test(`should test page content WCAG compliance for route - ${route}`, async ({
        page,
        makeAxeBuilder,
    }) => {
        ...
        const results = await makeAxeBuilder().include(`[data-test-id=${selector}]`).analyze();

        expect(results.violations).toEqual([]);
    });
});

test('should test page content WCAG compliance for home page', async ({
    page,
    baseURL,
    makeAxeBuilder,
}) => {
...
const sharedComponents = ['site_nav', 'site_header', 'site_footer'];
...
sharedComponents.forEach((component) => {
    test(`should test ${component} WCAG compliance`, async ({ page, makeAxeBuilder }) => {
        ...
        const results = await makeAxeBuilder().include(`[data-test-id=${component}]`).analyze();
