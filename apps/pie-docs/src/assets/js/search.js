...
document.querySelector('.DocSearch').onclick = () => {
    ...
    startScreenOptions.map(({ category, items }) => `<div key="${category.name}" class="DocSearch-NewStartScreenCategory">
        ...
        items.map(({ name, href }) => `<a href=${href} class="DocSearch-NewStartScreenItem">
            ${name}
          </a>`).join('')
