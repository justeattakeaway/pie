docsearch({
  ...
  const searchStartScreen = document.querySelector(".DocSearch").onclick = function() { 
  ...
  const startScreenOptions = [
    ...
    const result  =
      '<div class="DocSearch-NewStartScreen">' +
      startScreenOptions.map(({ category, items }) => {
          return `<div key="${category.name}" class="DocSearch-NewStartScreenCategory">
          ...
          items.map(({ name, href }) => {
            return `<a href=${href} class="DocSearch-NewStartScreenItem">
              ${name}
            </a>`
          }).join('')
