...
{% usage {
    ...
    dont: {
        type: usageTypes.text,
        items: [
            "Don't use Modal-native components for anything other than system prompted notifications. For other types of messages use the [custom Modal (Alert)](/components/modal/ios/) instead."
        ]
    }
} %}
